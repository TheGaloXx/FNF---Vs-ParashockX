...
class ChartingState extends MusicBeatState
{
	...
	*/
	...
	function reloadGridLayer() {
		...
		try
		{
			trace('Section beats: ${getSectionBeats()} - zoom: ${zoomList[curZoom]} - final height: ${GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]}');
			trace('Grid height: ${gridBG.height}.');
		}
		catch(e)
		{
		...
		var leHeight:Float = gridBG.height;
		...
		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + gridBG.width - (GRID_SIZE * 4)).makeGraphic(2, Std.int(leHeight), FlxColor.BLACK);
		...
		var gridBlackLine:FlxSprite = new FlxSprite(gridBG.x + GRID_SIZE).makeGraphic(2, Std.int(leHeight), FlxColor.BLACK);
		gridLayer.add(gridBlackLine);
		updateGrid();
